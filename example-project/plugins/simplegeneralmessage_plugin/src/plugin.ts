...
export class DummyPlugin implements Plugin<WorkflowPayload> {
  ...
  async handleWorkflow(
    ...
    let [recipient, destID, message] = ethers.utils.defaultAbiCoder.decode(["bytes32", "uint16", "string"], "0x" + hexPayload);
    recipient = this.formatAddress(recipient);
    const destChainID = destID as ChainId;
    this.logger.info(`VAA: sent "${message}" to ${recipient} on chain ${destID}.`);
